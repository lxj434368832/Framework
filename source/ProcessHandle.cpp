#include "../include/ProcessHandle.h"
...
std::string ProcessHandle::GetModulePath(HMODULE module)
{
	TCHAR cpath[MAX_PATH] = { 0 };
	TCHAR cdrive[MAX_PATH] = { 0 };
	TCHAR cdir[MAX_PATH] = { 0 };
	GetModuleFileName(module, cpath, MAX_PATH);
	_splitpath_s(cpath, cdrive, MAX_PATH, cdir, MAX_PATH, nullptr, 0, nullptr, 0);
...
bool ProcessHandle::TerminateProcByPID(DWORD dwPID)
{
	...
	if (INVALID_HANDLE_VALUE == hProcess)
	{
		printf("get process %d handle failed!", dwPID);
		return false;
	}

	if (false == ::TerminateProcess(hProcess, 0))		//终止进程
	{
		printf("TerminateProcess failed! process PID:%d,error code:%d", dwPID, ::GetLastError());
		::CloseHandle(hProcess);
		return false;
	}
