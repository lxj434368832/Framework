...
)
{
	...
	unsigned char step5data[MAX_MESSAGE_LENGTH + 128] = {0};
	unsigned char step8data[64 + 128] = {0};
	int i;
	for (i = 0; i<64; i++)
	{
		k0[i] = 0x00;
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"k0", k0, 64);
#endif
	/* Step 4 */
	for (i = 0; i<64; i++)
	{
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"k0 xor ipad", k0xorIpad, 64);
#endif
	/* Step 5 */
	for (i = 0; i<64; i++)
	{
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"(k0 xor ipad) || text", step5data, data_length + 64);
#endif
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"Hash((k0 xor ipad) || text)", digest, 20);
#endif
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"(k0 xor opad)", step7data, 64);
#endif
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"(k0 xor opad) || Hash((k0 xor ipad) || text)", step8data, 20 + 64);
#endif
...
#ifdef HMAC_DEBUG
	debug_out((unsigned char*)"HASH((k0 xor opad) || Hash((k0 xor ipad) || text))", digest, 20);
#endif
}