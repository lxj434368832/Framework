...
void configure::_resort_cfg()
{
    ...
    { 
		...
		if (cur_sec->find("FILENAME") != cur_sec->end())
		{
			...
			base::utils::strings::replace_all(block_[level::global].dst_file_format, base::consts::line_specifier, "");
		}
    ...
    for (auto &iter : cfg_section_list_)
    {
		...
		if (iter.second.find("FILENAME") != iter.second.end())
		{
			...
			base::utils::strings::replace_all(block_[cur_sec_lv].dst_file_format, base::consts::line_specifier, "");
		}
        else
...
void container::write_log(std::string log_name)
{
    ...
    for (auto iter : log_list_)
    {
        if (log_format_ != "" && log_format_.find(zxl::base::consts::format_specifier_char) != std::string::npos)
        {
            ...
            {
                base::utils::strings::replace_all(log_format, base::consts::line_specifier, helpers::get_app_name());
            }
            if (log_format.find(zxl::base::consts::msg_specifier) != std::string::npos)
            {
                base::utils::strings::replace_all(log_format, base::consts::msg_specifier, iter);
            }
...
std::string format_parser::parse(format_parser &&format, bool is_parse_all)
{
    ...
    {
        base::utils::strings::replace_all(format.log_format_, base::consts::date_specifier, base::utils::date_time::get_datetime(format.date_format_.c_str()));
    }
	if (format._has_flag(format_flag::ff_level))
	{
		base::utils::strings::replace_all(format.log_format_, base::consts::level_specifier, helpers::lv_convert_to_str(format.lv_));
	}
	if (format._has_flag(format_flag::ff_location))
	{
		base::utils::strings::replace_all(format.log_format_, base::consts::location_specifier, helpers::get_module_dir());
	}
	if (format._has_flag(format_flag::ff_app))
	{
		base::utils::strings::replace_all(format.log_format_, base::consts::app_specifier, helpers::get_app_name());
	}
    return format.log_format_;
}
...
void format_parser::_parse_format()
{
    ...
    if (log_format_.find(base::consts::level_specifier) != std::string::npos)
    {
        ...
        base::utils::strings::replace_all(log_format_, base::consts::level_specifier, helpers::lv_convert_to_str(lv_));
    }
    ...
    if (log_format_.find(base::consts::app_specifier) != std::string::npos)
    {
        ...
        base::utils::strings::replace_all(log_format_, base::consts::app_specifier, app_name);
    }
    if (log_format_.find(base::consts::location_specifier) != std::string::npos)
    {
        ...
        base::utils::strings::replace_all(log_format_, base::consts::location_specifier, location);
...
bool base::utils::strings::is_digit(char c)
{
    return c >= '0' && c <= '9';
}

bool base::utils::strings::wild_card_match(const char* str, const char* pattern) {
    while (*pattern) {
        switch (*pattern) {
        case '?':
            if (!*str)
...
std::string& base::utils::strings::ltrim(std::string& str) {
    str.erase(str.begin(), std::find_if(str.begin(), str.end(), std::not1(std::ptr_fun<int, int>(&std::isspace))));
    return str;
}

std::string& base::utils::strings::rtrim(std::string& str) {
    str.erase(std::find_if(str.rbegin(), str.rend(), std::not1(std::ptr_fun<int, int>(&std::isspace))).base(), str.end());
    return str;
}

std::string& base::utils::strings::trim(std::string& str) {
    return ltrim(rtrim(str));
}

bool base::utils::strings::starts_with(const std::string& str, const std::string& start) {
    return (str.length() >= start.length()) && (str.compare(0, start.length(), start) == 0);
}

bool base::utils::strings::ends_with(const std::string& str, const std::string& end) {
    return (str.length() >= end.length()) && (str.compare(str.length() - end.length(), end.length(), end) == 0);
}

std::string& base::utils::strings::replace_all(std::string& str, char replace_what, char replace_with) {
    std::replace(str.begin(), str.end(), replace_what, replace_with);
    return str;
}

std::string& base::utils::strings::replace_all(std::string& str, const std::string& replace_what, // NOLINT
...
void base::utils::strings::replace_first_with_escape(std::string& str, const std::string& replace_what, // NOLINT
    const std::string& replace_with) {
    std::size_t foundAt = std::string::npos;
    while ((foundAt = str.find(replace_what, foundAt + 1)) != std::string::npos) {
...
std::string& base::utils::strings::to_upper(std::string& str) {
    std::transform(str.begin(), str.end(), str.begin(), ::toupper);
    return str;
}

bool base::utils::strings::cstring_eq(const char* s1, const char* s2) {
    if (s1 == nullptr && s2 == nullptr) return true;
    if (s1 == nullptr || s2 == nullptr) return false;
    return strcmp(s1, s2) == 0;
}

bool base::utils::strings::cstring_case_eq(const char* s1, const char* s2) {
    if (s1 == nullptr && s2 == nullptr) return true;
    if (s1 == nullptr || s2 == nullptr) return false;
...
bool base::utils::strings::contains(const char* str, char c) {
    for (; *str; ++str) {
        if (*str == c)
            return true;
    }
...
char* base::utils::strings::convert_and_add_to_buff(std::size_t n, int len, char* buf, const char* bufLim, bool zeroPadded) {
    char localBuff[10] = "";
...
char* base::utils::strings::add_to_buff(const char* str, char* buf, const char* bufLim) {
    while ((buf < bufLim) && ((*buf = *str++) != '\0'))
        ++buf;
    return buf;
}

char* base::utils::strings::clear_buff(char buff[], std::size_t lim) {
    memcpy(buff, "", lim);
    return buff;
}

char* base::utils::strings::wcharptr_to_charptr(const wchar_t* line) {
    ...
    for (; *format; ++format) 
    {
        if (*format == base::consts::format_specifier_char) 
        {
            ...
            case '\0':  // End
                ...
                break;
            case 'd':  // Day
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_mday, 2, buf, buf_lim);
                continue;
            case 'a':  // Day of week (short)
				buf = base::utils::strings::add_to_buff(base::consts::days_abbrev[info->tm_wday], buf, buf_lim);
                continue;
            case 'A':  // Day of week (long)
				buf = base::utils::strings::add_to_buff(base::consts::days[info->tm_wday], buf, buf_lim);
                continue;
            case 'M':  // month
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_mon + 1, 2, buf, buf_lim);
                continue;
            case 'b':  // month (short)
				buf = base::utils::strings::add_to_buff(base::consts::months_abbrev[info->tm_mon], buf, buf_lim);
                continue;
            case 'B':  // month (long)
				buf = base::utils::strings::add_to_buff(base::consts::months[info->tm_mon], buf, buf_lim);
                continue;
            case 'y':  // year (two digits)
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_year + base::consts::year_base, 2, buf, buf_lim);
                continue;
            case 'Y':  // year (four digits)
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_year + base::consts::year_base, 4, buf, buf_lim);
                continue;
            case 'h':  // hour (12-hour)
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_hour % 12, 2, buf, buf_lim);
                continue;
            case 'H':  // hour (24-hour)
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_hour, 2, buf, buf_lim);
                continue;
            case 'm':  // minute
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_min, 2, buf, buf_lim);
                continue;
            case 's':  // second
				buf = base::utils::strings::convert_and_add_to_buff(info->tm_sec, 2, buf, buf_lim);
                continue;
            case 'z':  // milliseconds
            case 'g':
				buf = base::utils::strings::convert_and_add_to_buff(msec, 3, buf, buf_lim);
                continue;
            case 'F':  // AM/PM
				buf = base::utils::strings::add_to_buff((info->tm_hour >= 12) ? base::consts::kpm : base::consts::kam, buf, buf_lim);
                continue;
            default:
                continue;
            }
